...
**Result:** 580/580 passed, 0 failed
...
| TC-495 | Schema | A current database reports no gaps | schema_gaps() | 0 | 0 | PASS |
| TC-496 | Schema | Upgrading a current database changes nothing | upgrade_schema() | 0 | 0 | PASS |
| TC-497 | Schema | An older database is detected as behind | 4 tables + 1 column missing | True | as expected | PASS |
| TC-498 | Schema | Without the upgrade it reports 503, not a bare 500 | GET /api/bootstrap | 503 | 503 | PASS |
| TC-499 | Schema | and names the problem | error | schema_outdated | schema_outdated | PASS |
| TC-500 | Schema | and says exactly what to run | message | True | as expected | PASS |
| TC-501 | Schema | The upgrade adds the missing tables | 4 tables | True | as expected | PASS |
| TC-502 | Schema | and the missing column | overheads.spend_date | True | as expected | PASS |
| TC-503 | Schema | with nothing going wrong | problems | 0 | 0 | PASS |
| TC-504 | Schema | No gaps are left afterwards | schema_gaps() | 0 | 0 | PASS |
| TC-505 | Schema | Sign-in works once the database is upgraded | GET /api/bootstrap | 200 | 200 | PASS |
| TC-506 | Schema | The existing overhead survived untouched | ₹25,000 rent still there | 25000.0 | 25000.0 | PASS |
| TC-507 | Schema | and gained the new field as undated | dated flag | False | False | PASS |
| TC-508 | Schema | Every module answers on the upgraded database | 5 endpoints | [200, 200, 200, 200, 200] | [200, 200, 200, 200, 200] | PASS |
| TC-509 | Schema | Re-running the upgrade is a no-op | second run | 0 | 0 | PASS |
| TC-510 | Branches | Create with an explicit code | code=BX1 | BX1 | BX1 | PASS |
| TC-511 | Branches | Duplicate code is refused | code=BX1 again | 409 | 409 | PASS |
| TC-512 | Branches | Blank name is refused | name='' | 422 | 422 | PASS |
| TC-513 | Branches | Auto code is allocated when none is given | no code | True | as expected | PASS |
| TC-514 | Branches | Scales to any number (adds 15 at once, codes stay unique) | create 15 more branches | True | as expected | PASS |
| TC-515 | Branches | Rename works | PUT name | Renamed Hub | Renamed Hub | PASS |
| TC-516 | Branches | Deleting cascades to its records | DELETE BX1 | True | as expected | PASS |
| TC-517 | Branches | Cannot delete the last remaining branch | delete down to one | 409 | 409 | PASS |
| TC-518 | Users | Create a supervisor with a branch | role=supervisor | 201 | 201 | PASS |
| TC-519 | Users | Supervisor without a branch is refused | branches=[] | 422 | 422 | PASS |
| TC-520 | Users | Duplicate username is refused | username=tsup | 409 | 409 | PASS |
| TC-521 | Users | Unknown role is refused | role=owner | 422 | 422 | PASS |
| TC-522 | Users | New account can sign in | tsup/pw1234 | 200 | 200 | PASS |
| TC-523 | Users | Password reset takes effect | reset then login | 200 | 200 | PASS |
| TC-524 | Users | Too-short password is refused | pw='abc' | 422 | 422 | PASS |
| TC-525 | Users | Admin cannot delete their own account | self delete | 409 | 409 | PASS |
| TC-526 | Users | Deleted account can no longer sign in | delete tsup | 401 | 401 | PASS |
| TC-527 | Settings | Waste percentages are configurable | broiler 28% | 28.0 | 28.0 | PASS |
| TC-528 | Settings | New waste % feeds the calculation | 28% -> 72% yield | 72000 | 72000 | PASS |
| TC-529 | Settings | Restore the default | broiler 31% | 31.0 | 31.0 | PASS |
| TC-530 | Activity log | Records 'Sign in' | after the run above | True | as expected | PASS |
| TC-531 | Activity log | Records 'Failed sign in' | after the run above | True | as expected | PASS |
| TC-532 | Activity log | Records 'Submitted entry' | after the run above | True | as expected | PASS |
| TC-533 | Activity log | Records 'Approved entry' | after the run above | True | as expected | PASS |
| TC-534 | Activity log | Records 'Returned entry' | after the run above | True | as expected | PASS |
| TC-535 | Activity log | Records 'Added worker' | after the run above | True | as expected | PASS |
| TC-536 | Activity log | Records 'Created branch' | after the run above | True | as expected | PASS |
| TC-537 | Activity log | Records 'Added overhead' | after the run above | True | as expected | PASS |
| TC-538 | Activity log | Records 'Changed settings' | after the run above | True | as expected | PASS |
| TC-539 | Activity log | Records 'Created user' | after the run above | True | as expected | PASS |
| TC-540 | Activity log | Records 'Blocked: admin only' | after the run above | True | as expected | PASS |
| TC-541 | Activity log | Captures who did it | userName present | True | as expected | PASS |
| TC-542 | Activity log | Captures the role | role present | True | as expected | PASS |
| TC-543 | Activity log | Filter by action works | ?action=Sign in | True | as expected | PASS |
| TC-544 | Activity log | Supervisor cannot read it | GET as supervisor | 403 | 403 | PASS |
| TC-545 | Activity log | Blocked attempts are themselves logged | 'Blocked: admin only' | True | as expected | PASS |
| TC-546 | Robustness | Malformed JSON body does not crash | no body on login | 401 | 401 | PASS |
| TC-547 | Robustness | Missing fields default to zero | empty entry payload | 201 | 201 | PASS |
| TC-548 | Robustness | Negative weights are stored as given, not crashed | openWtG = -5000 | 201 | 201 | PASS |
| TC-549 | Robustness | Text in a numeric field is refused cleanly (422, not 500) | openBirds='abc' | 422 | 422 | PASS |
| TC-550 | Robustness | Very long note is truncated, not rejected | 3000 chars | True | as expected | PASS |
| TC-551 | Robustness | HTML in a note is stored safely as text | <script>alert(1)</script> | True | as expected | PASS |
| TC-552 | Robustness | Unknown branch code is refused | branch='ZZZ' | 403 | 403 | PASS |
| TC-553 | Robustness | Invalid category falls back to broiler | category='duck' | broiler | broiler | PASS |
| TC-554 | Robustness | A constraint breach returns 409, never 500 | duplicate day | 409 | 409 | PASS |
| TC-555 | Data wipe | There is real data to delete after the whole suite | entries > 0 | True | as expected | PASS |
| TC-556 | Data wipe | A supervisor cannot see the preview | GET wipe-preview | 403 | 403 | PASS |
| TC-557 | Data wipe | A supervisor cannot fetch the backup | GET wipe-backup | 403 | 403 | PASS |
| TC-558 | Data wipe | A supervisor cannot wipe | POST wipe | 403 | 403 | PASS |
| TC-559 | Data wipe | Preview reports what would be deleted | delete.entries | True | as expected | PASS |
| TC-560 | Data wipe | The backup carries every entry the preview counted | len(backup.entries) | 32 | 32 | PASS |
| TC-561 | Data wipe | Backed-up entries carry no photo data, just a count | photos empty, photoCount present | True | as expected | PASS |
| TC-562 | Data wipe | Preview reports what would be kept | keep.branches | 1 | 1 | PASS |
| TC-563 | Data wipe | No confirmation phrase is refused | POST with no body | 422 | 422 | PASS |
| TC-564 | Data wipe | A wrong confirmation phrase is refused | confirm='yes' | 422 | 422 | PASS |
| TC-565 | Data wipe | Nothing was deleted by the failed attempts | entries unchanged | 32 | 32 | PASS |
| TC-566 | Data wipe | The real wipe reports ok | ok | True | as expected | PASS |
| TC-567 | Data wipe | entries table is empty | daily_entries | 0 | 0 | PASS |
| TC-568 | Data wipe | purchases table is empty | purchases | 0 | 0 | PASS |
| TC-569 | Data wipe | hotelSales table is empty | customer_sales | 0 | 0 | PASS |
| TC-570 | Data wipe | payments table is empty | customer_payments | 0 | 0 | PASS |
| TC-571 | Data wipe | adjustments table is empty | customer_adjustments | 0 | 0 | PASS |
| TC-572 | Data wipe | overheads table is empty | overheads | 0 | 0 | PASS |
| TC-573 | Data wipe | dayCloses table is empty | day_close | 0 | 0 | PASS |
| TC-574 | Data wipe | labourLedger table is empty | labour_ledger | 0 | 0 | PASS |
| TC-575 | Data wipe | mortalityPhotos table is empty | mortality_photos | 0 | 0 | PASS |
| TC-576 | Data wipe | Branches are untouched | same count | 1 | 1 | PASS |
| TC-577 | Data wipe | User accounts are untouched | same count | 3 | 3 | PASS |
| TC-578 | Data wipe | Worker profiles are untouched | same count | 11 | 11 | PASS |
| TC-579 | Data wipe | Customer master records are untouched | same count | 8 | 8 | PASS |
| TC-580 | Data wipe | A second wipe finds nothing left to delete | delete.entries | 0 | 0 | PASS |
